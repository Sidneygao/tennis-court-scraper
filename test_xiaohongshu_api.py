...
from app.scrapers.xiaohongshu_api import XiaohongshuAPIScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('xiaohongshu_api_test.log', encoding='utf-8'),
        logging.StreamHandler()
    ]
)

def test_xiaohongshu_api_scraper():
    """测试小红书API爬虫"""
    print("🎾 小红书API爬虫测试")
    print("=" * 50)
    ...
    # 保存测试结果
    result_file = f'xiaohongshu_api_test_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='小红书API爬虫测试')
